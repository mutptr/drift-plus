...
const std::wstring name = xorstr_(L"blackcipher64.aes");
...
#define ORIGIN_ASM	"9C 52 54 5A 48 83 C2 08 48 81 EA 08 00 00 00 48 87 14"
#define JMP_ASM		"FF 25 00 00 00 00 CC CC CC CC CC CC CC CC"

blackcipher::blackcipher()
{
	pid_ = util::find_process_by_name(name);
	PLOGI << xorstr_("ngs pid: ") << pid_;
	base_ = util::find_module_by_name(pid_, name);
...
bool blackcipher::operator()() const
{
	return alloc_;
}